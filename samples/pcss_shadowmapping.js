...
async function main() {
  ...
  const sceneMaterial = new Material(gl, {
    ...
    fragmentShader: glsl`
      ...
      float getOccludersDepth(sampler2D uShadowMap, vec2 center, float currentDepth, float spread) {
        float average = 0.0;
        float counter = 0.0;

        for (uint i = 0u; i < 20u; i++) {
          vec2 offset = vogel(i, 20u, 0.0) * spread;
          float sampledDepth = texture(uShadowMap, center + offset).x;

          if (sampledDepth < currentDepth) {
            average += sampledDepth;
            counter += 1.0;
          }
        }
      ...
      float getPenumbra(sampler2D uShadowMap, vec2 center, float currentDepth, float spread) {
        float bias = 0.005;
        float average = 0.0;

        for (uint i = 0u; i < 50u; i++) {
          vec2 offset = vogel(i, 50u, 0.0) * spread;
          float closestDepth = texture(uShadowMap, center + offset).x;

          if (currentDepth - bias > closestDepth) {
            average += closestDepth * ((2.0 * spread) / length(offset - center));
          }
        }
      ...
      float calcShadow(vec4 fragPosLightSpace) {
        ...
        float occluderDepth = getOccludersDepth(
          uShadowMap,
          projCoords.xy,
          currentDepth + bias,
          0.01
        ...
        float distance = 10.0 * (currentDepth - occluderDepth) / occluderDepth;
